...
if ($action === 'get') {
    $stmt = $pdo->prepare("SELECT user_id, first_name, last_name, username, account_status, is_logged_in FROM users WHERE parent_id = ? AND account_status != 'deleted'");
...
elseif ($action === 'add') {
    ...
    $stmt = $pdo->prepare("INSERT INTO users (first_name, last_name, username, password, role, parent_id, account_status) VALUES (?, ?, ?, ?, 'cashier', ?, 'active')");
    $stmt->execute([$data->first_name, $data->last_name, $data->username, $hashed, $admin_id]);
...
elseif ($action === 'reset_password') {
    $hashed = password_hash($data->password, PASSWORD_DEFAULT);
    $stmt = $pdo->prepare("UPDATE users SET password = ? WHERE user_id = ? AND parent_id = ?");
    $stmt->execute([$hashed, $data->user_id, $admin_id]);
    echo json_encode(["success" => true]);
}
